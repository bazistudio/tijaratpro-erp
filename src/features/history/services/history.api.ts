...
export const historyApi = {
  getHistory: async (filters: HistoryFilterParams): Promise<{ data: HistoryItem[], total: number }> => {
    const { data } = await axiosInstance.get('/api/history', { params: filters });
    return data;
  },

  getStats: async (): Promise<HistoryStats> => {
    const { data } = await axiosInstance.get('/api/history/stats');
    return data;
  },

  getLedgerTrace: async (id: string): Promise<LedgerTraceItem[]> => {
    const { data } = await axiosInstance.get(`/api/history/trace/${id}`);
    return data;
  }
};
