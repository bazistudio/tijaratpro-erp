...
export const MobileSidebar = ({ isOpen, setIsOpen }: MobileSidebarProps) => {
  ...
  return (
    <div className="relative z-[var(--z-modal)] lg:hidden">
      <div 
        ...
        <div className="relative mr-16 flex w-full max-w-xs flex-1">
          <div className="absolute left-full top-0 flex w-16 justify-center pt-5">
            <button type="button" className="-m-2.5 p-2.5" onClick={() => setIsOpen(false)}>
              ...
              <X className="h-6 w-6 text-text-primary" aria-hidden="true" />
            </button>
          </div>

          <div className="flex grow flex-col overflow-y-auto bg-surface pb-4">
            <div className="flex h-12 shrink-0 items-center px-6 border-b border-border sticky top-0 bg-surface z-10">
              <span className="text-lg font-bold text-primary">TijaratPro</span>
            </div>
                      ...
                      {permittedItems.map(item => {
                        ...
                        return (
                          <Link
                            ...
                            href={item.href}
                            onClick={() => setIsOpen(false)}
                            className={`group flex items-center px-2 py-1.5 text-sm font-medium rounded-md transition-all duration-fast ${
                              isActive
                          ...
                          >
                            <Icon
                              className={`flex-shrink-0 -ml-1 mr-3 h-4 w-4 transition-colors duration-fast ${
